+(function ($) {
    ...
    MarkdownEditor.prototype.makeToolbarButton = function (code, button) {
        if (
            !this.options.useMediaManager &&
            (code == "medialink" || code == "mediaimage")
        ) {
            return;
        }
    ...
    MarkdownEditor.prototype.createCodeContainer = function () {
        ...
        editor.getSession().$bidiHandler.$isRtl = true;
        ...
        assetManager.load(
            {
                js: [this.options.vendorPath + "/theme-github.js"],
            },
            function () {
    ...
    MarkdownEditor.prototype.setViewMode = function (value) {
        ...
        $('[data-button-code="preview"]', this.$toolbar).toggle(
            !this.isSplitMode
        );

        this.$el.removeClass("mode-tab mode-split").addClass("mode-" + value);
    ...
    MarkdownEditor.prototype.toggleFullscreen = function () {
        ...
        this.editor.focus();
        $('[data-button-code="fullscreen"]', this.$toolbar).toggleClass(
            "active"
        );
    };
    ...
    MarkdownEditor.prototype.formatInline = function (template) {
        var editor = this.editor,
            pos = this.editor.getCursorPosition(),
            text = editor.session
                .getTextRange(editor.selection.getRange())
                .trim();
    ...
    $.fn.markdownEditor = function (option) {
        ...
        items = this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.markdownEditor",
                    (data = new MarkdownEditor(this, options))
                );
